...
// Global Vars - Used for reading/saving IMU data
NineAxesMotion mySensor;             //Object that for the sensor

unsigned long startTime = 0;         // Find start time, so we can limit how long we read
int maxTime = 5;                    //Number of seconds to run code for
...
bool csv = true;                     // Flag to print data only for CSV use. If true, turns off printData later.
...
void printout(bool csv, bool printData);  //Prints out data in desired form, mostly to keep main loop uncluttered
void space();                   // Fancy print statement to shorten csv separators
...
void setup() //This code is executed once------------------------------------------------
{
  ...
  if (csv == false) //Don't print this is you're in csv mode
  {
    Serial.println();
  ...
  if (csv == true)
  {
    ...
    printData = false;     //If you're printing for a csv, do not print with titles
    // Print headers exactly once
    Serial.print("Time, ");
    Serial.print("aX, ");
    Serial.print("aY, ");
    Serial.print("aZ, ");
  ...
  startTime = millis();       //Declare us to have started, and take the time.
...
//----------------------Bump flagging. Commented until better graphed and understood.----------------
...
//    }
  ...
  if ((millis() - lastStreamTime) >= streamPeriod)
  {
    ...
    printout(csv, printData); //Print all readings, in format determined by csv and printData

    updateSensorData = true;
  }


  // Stops things after the preset maximum time
  if ((millis() - startTime) >= maxTime*1000)
  {
    // If we go over 2 seconds, stop
    while(1){}; // hang the program so it stops doing anything
  }
}
...
void printout(bool csv, bool printData)
{
    if (csv == true)
  {
    Serial.print(lastStreamTime); space();
    Serial.print(ax); space(); //Accelerometer X-Axis data
    Serial.print(ay); space();  //Accelerometer Y-Axis data
    Serial.print(az); space();  //Accelerometer Z-Axis data

    Serial.print(rx); space(); //Pitch data
    Serial.print(ry); space(); //Roll data
    Serial.print(rz); space(); //Heading data
...
// Simple print statement to simplify making things into csv-friendly format
void space()
{
  Serial.print(",     ");
}
